      ...
      <div className="container mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {solutions.map((solution) => (
            <div key={solution.name} className="bg-white rounded-lg shadow-lg overflow-hidden">
              <div className="p-6">
                <h3 className="text-xl font-semibold mb-4 bg-orange-400 text-white py-2 px-4 rounded-full inline-block">
                  {solution.name}
                </h3>
                <p className="text-gray-700 mb-4">{solution.description}</p>
                <Link href={`/${solution.name.toLowerCase()}`} className="text-gray-500 hover:text-orange-400">
                  Learn more
                </Link>
              </div>
            </div>
